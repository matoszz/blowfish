...
func init() {
	...
	systemCmd.Flags().StringArrayP("output", "o", nil, "Output format")
	viperBindFlag("get.output", systemCmd.Flags().Lookup("output"))
}
...
func getSystem(cmd *cobra.Command, args []string) error {
	...
	writer := utils.NewTableWriter(
		cmd.OutOrStdout(),
		"manufacturer", "model", "processor", "memory", "power", "status", "biosVersion")
	...
	srvJSON, err := json.Marshal(systems)
	if err != nil {
		return err
	}

	// validate output format
	output := viper.GetStringSlice("get.output")
	if len(output) == 0 {
