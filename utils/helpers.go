...
func GofishClient(connection string) (*gofish.APIClient, error) {
	...
	if settings == nil {
		return nil, config.NewArgumentError("unable to get system connection information.\nSet default to use or provide on command line with -c [NAME].", connection)
	}
	...
	if err != nil {
		return nil, config.NewArgumentError("failed to connect to", cfg.Endpoint)
	}
